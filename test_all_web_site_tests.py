"""Оформить в Page Object не успела..."""
...
def test_1_1_view_language_nav_bar(browser, link, languages_initial):
    ...
    assert len(browser.find_elements_by_xpath('//form[@id="language_selector"]/div/select[@name="language"]')) != 0, \
        'На главной странице отсутствует поле выбора языка с выпадающим списком.'

    assert len(browser.find_elements_by_xpath(
                             '//form[@id="language_selector"]/div/select[@name="language"]/option')) != 0, \
        'На главной странице отсутствуют элементы выпадающего списка при клике на поле выбора языка.'
    ...
    assert len(browser.find_elements_by_xpath('//form[@id="language_selector"]/button[@type="submit"]')) != 0, \
        'На главной странице отсутствует кнопка применения выбранного языка.'


@pytest.mark.need_review_custom_scenarios
def test_1_2_view_login_registration_link(browser, link):
    """Проверка наличия надписи-ссылки 'Войти или зарегистрироваться'"""

    browser.get(link)
    browser.implicitly_wait(10)

    assert len(browser.find_elements_by_xpath('//ul[@class="nav navbar-nav navbar-right"]/li/a[@id="login_link"]')) != 0, \
    ...
    assert len(browser.find_elements_by_xpath(
        '//ul[@class="nav navbar-nav navbar-right"]/li/a[@id="login_link"]/i[@class="icon-signin"]')) != 0, \
        'На главной странице отсутствует кликабельная стрелочка слева от надписи-ссылки ' \
...
def test_1_3_view_home_page_title(browser, link, current_language):
    ...
    assert len(browser.find_elements_by_xpath('//div[@class="col-sm-7 h1"]/a')) != 0, \
        'На главной странице отсутствует надпись-ссылка Oscar.'
    if current_language == 'ru':
        assert browser.find_element_by_xpath('//div[@class="col-sm-7 h1"]/a').text == 'Oscar'

    assert len(browser.find_elements_by_xpath('//div[@class="col-sm-7 h1"]/small')) != 0, \
        'На главной странице отсутствует заголовок Sandbox справа от Oscar.'
    if current_language == 'ru':
...
def test_1_4_view_store_drop_down(browser, link, current_language):
    ...
    assert len(browser.find_elements_by_xpath(
        '//div[@class="navbar-collapse primary-collapse collapse"]/ul[@class="nav navbar-nav"]')) != 0, \
        'На главной странице отсутствует элемент "Просмотр магазина".'

    assert len(browser.find_elements_by_xpath(
        '//li[@class="dropdown active open"]/a[@class="dropdown-toggle"]/b[@class="caret"]')) != 0, \
        'На главной странице отсутствует стрелочка справа от "Просмотр магазина".'
...
@pytest.mark.need_review_custom_scenarios
def test_1_5_view_cart_button(browser, link):
    ...
    assert len(browser.find_elements_by_xpath(
        '//div[@class="basket-mini pull-right hidden-xs"]/span[@class="btn-group"]')) != 0, \
        'На главной странице отсутствует кнопка "Посмотреть корзину".'

    assert len(browser.find_elements_by_xpath(
        '//span[@class="btn-group"]/button[@class="btn btn-default dropdown-toggle"]')) != 0, \
        'На главной странице отсутствует кнопка-стрелочка вниз справа от кнопки "Посмотреть корзину".'
...
def test_1_6_view_search_field_button(browser, link):
    ...
    assert len(browser.find_elements_by_xpath(
        '//form[@class="navbar-form navbar-right"]/div[@class="form-group"]/input[@type="search"]')) != 0, \
        'На главной странице отсутствует поисковое поле.'

    assert len(browser.find_elements_by_xpath('//form[@class="navbar-form navbar-right"]/input[@type="submit"]')) != 0, \
        'На главной странице отсутствует кнопка "Найти" справа от поискового поля.'


@pytest.mark.need_review_custom_scenarios
def test_1_7_view_login_registration_page(browser, link, current_language):
...
def test_1_9_view_all_products(browser, link):
    ...
    for letter in number_of_products:
        assert letter in [str(i) for i in range(10)], 'Возвращаемое значение элемента (общее количество товаров) ' \
                                                      'должно быть числом.'
    ...
    if number_of_products >= 20:
        ...
        for letter in start_number_of_products:
            assert letter in [str(i) for i in range(10)], 'Возвращаемое значение элемента (номер первого товара ' \
                                                          'в строке количества товаров на странице) должно быть числом.'
        ...
        for letter in end_number_of_products:
            assert letter in [str(i) for i in range(10)], 'Возвращаемое значение элемента (номер последнего товара ' \
                                                          'в строке количества товаров на странице) должно быть числом.'
...
def login(browser, link, current_language, input_email='true_email@test.ru', input_password='truepassw',
           input_new_password='truepassw1', login=True, pre_registration=True, repeat_login=False,
    ...
    def login_pass(input_email=input_email, input_password=input_password, skip_check=False):
        # Переход по ссылке 'Войти или зарегистрироваться'
        login_registration_link = browser.find_element_by_xpath(
            '//ul[@class="nav navbar-nav navbar-right"]/li/a[@id="login_link"]')
        login_registration_link.click()
        ...
        login_email_field = browser.find_element_by_xpath('//input[@name="login-username"]')
        login_email_field.clear()
        sleep(1)
        login_email_field.send_keys(input_email)

        # Ввести пароль в форме авторизации
        login_password_field = browser.find_element_by_xpath('//input[@name="login-password"]')
        login_password_field.clear()
        sleep(1)
        login_password_field.send_keys(input_password)

        # Нажать кнопку Войти"
        login_button = browser.find_element_by_xpath('//button[@name="login_submit"]')
        browser.implicitly_wait(10)
        login_button.click()
        ...
        if not skip_check:
            if login:
                ...
                success_login_msg = browser.find_element_by_xpath(
                    '//div[@id="messages"]/div[@class="alert alert-success  fade in"]/div[@class="alertinner wicon"]')
                if current_language == 'ru':
                    assert success_login_msg.text == 'Рады видеть вас снова', 'Сообщение об успешной аутентификации:' \
                                                                               '\n""{}"\nне совпадает с ожидаемым.' \
                                                                               ''.format(success_login_msg.text)
                # Иконка аккаунта
                browser.find_element_by_xpath('//i[@class="icon-user"]')
                ...
                login_error_msg = browser.find_element_by_xpath('//div[@class="alert alert-danger"]')
                if current_language == 'ru':
                    assert login_error_msg.text == 'Опаньки! Мы нашли какие-то ошибки - пожалуйста, ' \
                                                    'проверьте сообщения об ошибках ниже и попробуйте еще раз', \
                        'Сообщение об ошибке аутентификации:\n{}\nНе совпадает с ожидаемым.' \
                        ''.format(login_error_msg.text)
    ...
    if login:
        if repeat_login:
            ...
            if new_password:
                ...
                exit_account.click()
                sleep(1)
                login_pass(input_password=input_new_password)
            else:
        ...
        login_pass()
        login_pass(input_email='true_email@test.ru', input_password='truepassw', skip_check=True)
        delete_profile()


def add_product_to_cart_and_clear_cart(browser, link, clear_cart=False, add_two_products=False,
    ...
    def delete_products_using_delete_button():
        ...
        number_of_products_in_cart = len(browser.find_elements_by_xpath('//div[@class="basket-items"]'))
        ...
        if del_one:
            ...
            delete_button = browser.find_element_by_xpath(
                ...
                ''.format(number_of_products_in_cart))
            delete_button.click()
        ...
        if del_one:
            assert number_of_products_after == \
                   number_of_products_in_cart - 1, 'Был удалён 1 товар из корзины, используя кнопку \'Удалить\'. ' \
                                                     'Количество товаров в корзине до удаления={}.\n Количество товаров ' \
                                                     'после удаления одного товара={} и не соответствует ожидаемому.' \
                                                     ''.format(number_of_products_in_cart, number_of_products_after)
        else:
    ...
    def delete_products_using_clear():
        ...
        if del_one:
            assert number_of_products_after == \
                   number_of_products_in_cart - 1, 'Был удалён 1 товар из корзины, используя запись \'0\' в поле ' \
    ...
    if del_products_using_delete_button:
        ...
        if clear_cart or repeat_add_product:
...
@pytest.mark.need_review_custom_scenarios
def test_2_7_login_true_data(browser, link, current_language):
    """Аутентификация пользователя. Зарегистрированный email, корректный пароль."""

    # input_email='true_email@test.ru', input_password='truepassw'
    login(browser, link, current_language)


@pytest.mark.need_review_custom_scenarios
def test_2_8_repeat_login(browser, link, current_language):
    """Аутентификация пользователя. Выход из профиля. Повторная аутентификация."""

    # input_email='true_email@test.ru', input_password='truepassw'
    login(browser, link, current_language, repeat_login=True)


@pytest.mark.need_review_custom_scenarios
def test_2_9_login_false_email(browser, link, current_language):
    """Аутентификация пользователя. Незарегистрированный email, корректный пароль."""

    # input_email=False, input_password='truepassw'
    login(browser, link, current_language, input_email='false_email@test.ru', login=False)


@pytest.mark.need_review_custom_scenarios
def test_2_10_login_false_password(browser, link, current_language):
    ...
    login(browser, link, current_language, input_password='falsepassw', login=False)
...
def test_2_11_change_password_true_data(browser, link, current_language):
    ...
    login(browser, link, current_language, change_password=True)
...
def test_2_12_change_password_false_data(browser, link, current_language):
    ...
    login(browser, link, current_language, change_password=True, new_password=False)
...
@pytest.mark.need_review_custom_scenarios
def test_2_14_add_one_product_to_cart(browser, link):
    """Добавить 1 товар в корзину со страницы всех товаров"""

    add_product_to_cart_and_clear_cart(browser, link)


@pytest.mark.need_review_custom_scenarios
def test_2_15_add_one_product_to_cart_from_product_page(browser, link):
    """Добавить 1 товар в корзину со страницы с описанием товара"""

    add_product_to_cart_and_clear_cart(browser, link, add_from_product_page=True)


@pytest.mark.xfail
@pytest.mark.need_review_custom_scenarios
def test_2_16_clear_cart_with_one_product_using_delete_button(browser, link):
    """Очистить корзину с одним товаром, используя кнопку 'Удалить'."""

    add_product_to_cart_and_clear_cart(browser, link, clear_cart=True, del_products_using_delete_button=True)


@pytest.mark.need_review_custom_scenarios
def test_2_17_clear_cart_with_one_product(browser, link):
    """Очистить корзину с одним товаром, используя запись '0' в поле количества и кнопку 'Обновить'."""

    add_product_to_cart_and_clear_cart(browser, link, clear_cart=True)


@pytest.mark.need_review_custom_scenarios
def test_2_18_repeat_add_one_product_to_cart(browser, link):
    """Повторно добавить товар в корзину."""

    add_product_to_cart_and_clear_cart(browser, link, repeat_add_product=True)


@pytest.mark.need_review_custom_scenarios
def test_2_19_add_two_products_to_cart(browser, link):
    """Добавить 2 товарова в корзину."""

    add_product_to_cart_and_clear_cart(browser, link, add_two_products=True)


@pytest.mark.need_review_custom_scenarios
def test_2_20_add_several_products_to_cart(browser, link):
    """Добавить несколько товаров в корзину."""

    add_product_to_cart_and_clear_cart(browser, link, add_several_products=True)


@pytest.mark.xfail
@pytest.mark.need_review_custom_scenarios
def test_2_21_del_one_from_several_products_in_cart_using_delete_button(browser, link):
    """Удалить один из нескольких товаров в корзине, используя кнопку 'Удалить'."""

    add_product_to_cart_and_clear_cart(browser, link, clear_cart=True, add_several_products=True,
                                           del_one=True, del_products_using_delete_button=True)


@pytest.mark.xfail
@pytest.mark.need_review_custom_scenarios
def test_2_22_clear_cart_with_several_products_using_delete_button(browser, link):
...
@pytest.mark.need_review_custom_scenarios
def test_2_23_del_one_from_several_products_in_cart(browser, link):
...
@pytest.mark.need_review_custom_scenarios
def test_2_24_clear_cart_with_several_products(browser, link):
